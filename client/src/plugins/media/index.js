...
const MQ = AppConstant.MATCH_MEDIA.MQ
const MQMAP = AppConstant.MATCH_MEDIA.MQMAP
  ...
  Vue.directive('onmedia', {
    bind (el, { value, expression, arg, modifiers }, { context }) {
      ...
      if (!(value instanceof Function)) {
        Vue.util.warn(`Error binding v-onmedia: expression "${expression}" doesn't resolve to a component method, so there's nothing to call back on change`, context)
        return
      }
      if (NOT) {
        if (ANY) {
